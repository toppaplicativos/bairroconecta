...
const AttendeeAvatar = ({ src, index }: { src: string, index: number }) => (
    <div className="w-6 h-6 rounded-full overflow-hidden border-2 border-background" style={{ marginLeft: index > 0 ? '-8px' : '0px' }}>
        <Image src={src} alt="Participante" width={24} height={24} className="object-cover" />
    </div>
);
        ...
        <CardContent className="p-3">
          ...
          <div className="text-sm text-muted-foreground space-y-1.5 mt-2">
              ...
              <div className="flex items-center gap-2">
               ...
               <div className="flex items-center gap-2">
                  <Ticket className="h-4 w-4 text-primary -rotate-45" />
                  <span className="font-bold text-primary">R${event.price} <span className="text-muted-foreground font-normal">/Pessoa</span></span>
                   <div className="flex items-center ml-auto">
                      {event.attendees.slice(0, 3).map((attendee, index) => (
                          <AttendeeAvatar key={index} src={attendee} index={index} />
                      ))}
                      {event.attendees.length > 3 && (
                          <div className="w-6 h-6 rounded-full bg-primary text-white flex items-center justify-center text-xs font-bold border-2 border-background" style={{ marginLeft: '-8px' }}>
                             +{event.attendees.length - 3}
                          </div>
                      )}
                  </div>
