...
const tones = {
  ...
  amber: 'bg-amber-500/10 text-amber-700 ring-amber-500/15 dark:text-amber-400',
...
export function ModuleCard({
  ...
  return (
    <Link
      href={href}
      className="group relative overflow-hidden rounded-2xl border border-border/80 bg-card p-5 shadow-panel transition duration-200 hover:-translate-y-0.5 hover:border-border hover:shadow-float focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40"
    >
      ...
      {meta ? (
        <div className="mt-5 border-t border-border/70 pt-4 text-xs font-semibold text-muted-foreground">
          {meta}
        </div>
      ) : null}
    </Link>
